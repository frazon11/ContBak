...
Version 1.2.0 restores mount contents into the original container's currently configured mounts. The container must still exist. Restore removes the existing content of every selected mount before extracting its archive.
...
```bash
git tag -a v1.2.0 -m "ContBak 1.2.0"
