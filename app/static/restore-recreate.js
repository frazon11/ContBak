(() => {
  ...
  function ensureStyles(){
    ...
    const s=document.createElement('style');s.id='restore-recreate-style';s.textContent=`
      ...
      .restore-summary{padding:12px;border:1px solid var(--line);border-radius:9px;background:var(--surface-2);font-size:12px}.restore-summary strong{display:block;margin-bottom:5px}.restore-actions{display:flex;justify-content:flex-end;gap:9px;margin-top:18px}.restore-warning{color:#fcd34d!important}
  ...
  function choose(info){
    return new Promise(resolve=>{
      ...
      bg.innerHTML=`<div class="restore-modal" role="dialog" aria-modal="true">
        ...
        <div class="restore-summary"><strong>${escapeHtml(info.original_name||'—')}</strong>${tx('Image','Image')}: ${escapeHtml(info.image||'—')}<br>${tx('Restorable mounts','Wiederherstellbare Mounts')}: ${(info.mounts||[]).filter(m=>m.archive).length}</div>
        ...
        <div class="restore-actions"><button class="btn btn-secondary" id="rr-cancel">${tx('Cancel','Abbrechen')}</button><button class="btn btn-danger" id="rr-start">${tx('Start restore','Restore starten')}</button></div>
      </div>`;
